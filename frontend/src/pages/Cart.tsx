...
const Cart = () => {
    ...
    const handleCheckout = async (details: { name: string; phone: string; transactionId: string }) => {
        ...
        try {
            // Create orders for each item sharing the same details
            // Note: Ideally we'd have an 'orders' table and 'order_items' table.
            // Given current schema, we insert multiple rows into 'orders'. 
            // We'll generate a single batch ID if possible or just share transaction ID.

            // We will loop and insert.
            const promises = items.map(item =>
                supabase.from("orders").insert({
                    order_id: "", // Let DB handle or generate unique if needed, but schema says NOT NULL. client.ts says generate_order_id function exists?
                    // Wait, typically order_id is generated by DB default or trigger. 
                    // Let's check Payment.tsx again. It passed empty string "".
                    name: details.name,
                    phone: details.phone,
                    dessert_id: item.id,
            ...
            if (errors.length > 0) {
                throw new Error("Some items failed to order: " + errors[0].error?.message);
            }

            // Just take the first order ID for display
            const firstOrderId = results[0].data?.[0]?.order_id;
            ...
            if (firstOrderId) {
                ...
                // Fallback if no ID returned (shouldn't happen with select)
